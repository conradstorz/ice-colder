...
from vmc_core import VMCCore, STATE_CHANGE_PREFIX
...
class VMC(VMCCore):
    def __init__(self, config: ConfigModel):
        ...
        self.products = self.config_model.physical_details.products
        self.owner_contact = self.config_model.machine_owner_contact
    ...
    def initiate_virtual_payment(self, amount, tk_root):
        ...
        if not gateways:
            ...
            self.send_customer_message(
                "Virtual payment is currently unavailable.", tk_root
            )
            return
        ...
        if self.qrcode_callback:
            logger.debug(
                "QR code callback is set; updating UI with generated QR code image."
            )
    ...
    def select_product(self, product_index, tk_root):
        ...
        if (
            self.selected_product.get("track_inventory", False)
            and self.selected_product.get("inventory_count", 0) <= 0
        ):
            logger.error(f"{self.selected_product.get('name')} is sold out.")
            self.send_customer_message(
                f"{self.selected_product.get('name')} is sold out. Please select another product.",
                tk_root,
            )
            return
    ...
    def _process_payment(self, tk_root):
        ...
        if self.credit_escrow >= price:
            ...
            self.send_customer_message(
                "Sufficient funds received. Processing your payment...", tk_root
            )
