...
class PaymentDeviceFSM(ABC):
    ...
    def __init__(self, device_name, callback=None):
        ...
        logger.debug(
            f"{self.device_name} FSM: __init__ called. Callback set: {bool(callback)}"
        )
    ...
    def register_callback(self, callback):
        ...
        logger.debug(
            f"{self.device_name} FSM: register_callback called. Callback updated."
        )
    ...
    def notify(self, event_type, data):
        ...
        """
        logger.debug(
            f"{self.device_name} FSM: notify called with event_type='{event_type}', data={data}"
        )
        logger.info(
            f"{self.device_name} FSM: Notifying event '{event_type}' with data: {data}"
        )
        if self.callback:
            logger.debug(
                f"{self.device_name} FSM: Executing callback for event '{event_type}'."
            )
            try:
                self.callback(event_type, data)
                logger.debug(
                    f"{self.device_name} FSM: Callback executed successfully for event '{event_type}'."
                )
            except Exception as e:
                logger.exception(
                    f"{self.device_name} FSM: Exception in callback for event '{event_type}': {e}"
                )
        else:
            logger.warning(
                f"{self.device_name} FSM: No callback registered; event '{event_type}' not delivered."
            )
